...
function getGroupDelimToken(groupType) {
    return {
        token_type: 'Delim',
        data: groupType === 'Unenclosed' || groupType === 'Curly'
            ? [';', 'Semi']
            : [',', 'Comma'],
    };
}
        ...
        for (let i = 0; i < trees.length; i++) {
            ...
            if (ignoringNextStatement) {
                ...
                if (!isDelim || i !== trees.length - 1) {
                    resultArray.push(isDelim /* && options.replaceComma */
                ...
                if (i < trees.length - 1) {
                    ...
                    resultArray.push(printBetween(trees, i, i + 1, leftMap, rightMap));
                }
                else if (results.length || resultGroup.length) {
                    endGroup();
                    // Trailing delimiter
                    if ((!isSeparator || isDelim) &&
                        !hasNestedGroup &&
                        !shouldKeepSameLine() &&
                        ...
                        results.push(ifBreak(printToken(getGroupDelimToken(groupType))));
                    }
                }
            }
        }
...
function printBetween(trees, aIndex, bIndex, 
// a: TokenTree,
// b: TokenTree,
leftMap, rightMap) {
    const rule = spaceConfig_1.default.rules.find(([aPattern, bPattern]) => {
        ...
        (0, spaceConfig_1.doesTokenTreeMatchPattern)(aPattern, trees, aIndex) &&
            (0, spaceConfig_1.doesTokenTreeMatchPattern)(bPattern, trees, bIndex));
    });
