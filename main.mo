...
shared ({ caller = owner }) actor class (
  ...
  public shared ({ caller }) func endTournament(tournamentId : Text) : async Result.Result<(), OT.Error> {
    ...
    switch (tournamentRes) {
      case (#ok(tournament)) {
        let internalStats : [OT.PlayerStatsSuccess] = await getInternalStats();
  ...
  private func _initTournament() : async Result.Result<(), OT.TournamentError> {
    ...
    if (initialStatsRes.size() != 0) {
      label l for (playerStats in initialStatsRes.vals()) {
  ...
  private func getPlayersStats() : async [(Principal, IT.PlayerStats)] {
    //TODO: agregar module con canisterIds
    let bountyRushService = actor ("x6h7n-bqaaa-aaaan-qajxq-cai") : actor {
  ...
  private func getInternalStats() : async [OT.PlayerStatsSuccess] {

    var endStatsRes : [(Principal, IT.PlayerStats)] = await getPlayersStats();
    let internalResultsBuff : Buffer.Buffer<OT.PlayerStatsSuccess> = Buffer.Buffer(1);

    for (eS in endStatsRes.vals()) {
